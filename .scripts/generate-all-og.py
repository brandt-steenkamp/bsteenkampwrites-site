...
def build_article_url(article_path: Path) -> str:
    relative_path = article_path.relative_to(ROOT_DIR).as_posix()
    return f"{SITE_ROOT}/{relative_path}"


def find_title(article_html: str) -> str:
    return (
...
def main() -> None:
    ...
    satire_files = iter_satire_files()
    if not satire_files:
